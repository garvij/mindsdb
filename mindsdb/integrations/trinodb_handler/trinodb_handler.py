...
class TrinodbHandler(DatabaseHandler):
    ...
    def join(self, stmt, data_handler, into: Optional[str] = None) -> pd.DataFrame:
        ...
        TODO: Update this
        """
        local_result = self.select_query(stmt.targets, stmt.from_table.left,
                                         stmt.where)  # should check it's actually on the left
        external_result = data_handler.select_query(stmt.targets, stmt.from_table.right,
                                                    stmt.where)  # should check it's actually on the right
